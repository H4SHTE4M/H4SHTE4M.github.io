...
在存档文件夹中还发现一个压缩包，把`HDCTFWIN!`作为压缩包的key，解密获得一个svp文件。
...
- 本题的特殊之处在于，假设现在有栈指针 `A=>B=>C=>D` ，我可以用格式化字符通过 `B` 为跳板修改 `C` 为 `E`，那么修改后的链为 `A=>B=>E=>F`，如果有第二次格式化字符串漏洞的话，我可以找到链 `B=>E=>F` 通过 `E` 为跳板，修改原本的 `F` 为 `G`。但这个操作无法用一次的格式化字符串漏洞完成，可能因为 `B=>E=>F` 这条链本身是不存在的，即使用格式化字符串漏洞做出了 `B=>E=>F` 这条链也无法同时再去改这条链上的指针
